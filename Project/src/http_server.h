...
public:
    ...
    Session(tcp::socket&& socket, const std::string ip_address, Handler&& request_handler)
...
private:
    ...
    void HandleRequest(HttpRequest&& request) override {
        ...
        request_handler_(std::move(ip_address_), std::move(request), [self = this->shared_from_this()](auto&& response) {
...
private:
    ...
    void AsyncRunSession(tcp::socket&& socket) {
        std::make_shared<Session<RequestHandler>>(std::move(socket), socket.remote_endpoint().address().to_string(), request_handler_)->Run();
    }
